...
export default function ModalReport({ show, onClose, shopId }) {
  ...
  useEffect(() => {
    if (show && shopId) {
      setLoading(true);
      setError(null);
      fetch(`https://alfarhaapp.onrender.com/report/${shopId}`)
        .then((res) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl p-8 w-full max-w-3xl mx-4">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          <label htmlFor="date-from" className="text-white/80 font-semibold">
            Date from:
          </label>
          <input
            id="date-from"
          ...
          />
          <label htmlFor="date-to" className="text-white/80 font-semibold">
            to
          </label>
          <input
            id="date-to"
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <div className="text-center py-8 text-blue-200 font-semibold">
              Loading...
            </div>
          ) : error ? (
                ...
                <table className="min-w-full border border-white/20 rounded-xl overflow-hidden shadow-lg">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-gradient-to-r from-blue-700 via-purple-700 to-indigo-700 text-white">
                      <th className="px-4 py-3 border-b border-white/20 font-bold">
                        #
                      </th>
                      <th className="px-4 py-3 border-b border-white/20 font-bold">
                        Date
                  ...
                  <tbody>
                    ...
                    filteredData.length === 0 ? (
                      <tr>
                        <td
                          colSpan={6}
                          className="text-center py-6 text-blue-200 bg-white/5"
                        >
                          No reports found.
                        </td>
                      </tr>
                      ...
                      filteredData.map((row, idx) => (
                        ...
                        >
                          <td className="px-4 py-2 border-b border-white/10 text-center">
                            {idx + 1}
                          </td>
                          <td className="px-4 py-2 border-b border-white/10 text-center">
                            {row.date || row.started_at?.slice(0, 10) || "-"}
                          </td>
                          <td className="px-4 py-2 border-b border-white/10 text-center font-mono text-xs">
                            {row.gameId || row.round_id || "-"}
                          </td>
                          <td className="px-4 py-2 border-b border-white/10 text-center text-green-300 font-bold">
                            {row.prize || "-"}
                          </td>
                          <td className="px-4 py-2 border-b border-white/10 text-center">
