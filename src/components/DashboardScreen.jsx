...
export default function DashboardScreen({
  ...
  const speechUtteranceRef = useRef(new SpeechSynthesisUtterance());
  const audioContextRef = useRef(null); // will hold AudioContext
  ...
  useEffect(() => {
    ...
    for (const [cat, [start, end]] of Object.entries(ranges)) {
      for (let i = start; i <= end; i++) {
        const path = `/game/arabic/${cat}${i}.mp3`;
  ...
  const playSoundForCall = (category, number) => {
    ...
    const audioPath = `/game/arabic/${category}${number}.mp3`; // match preloaded files
    ...
    if (isArabic) {
      ...
      let audioInstance = audioCache.current.get(audioPath);
      if (!audioInstance) {
        audioInstance = new Audio(audioPath);
        audioCache.current.set(audioPath, audioInstance);
      }
      ...
      audioRef.current
        .play()
        .catch((err) => console.warn("🎧 Arabic audio play error:", err));
    } else {
      // English TTS
      const message = `${category} ${number}`;
  ...
  const togglePlayPause = () => {
    // 🎮 Toggle game state first
    setIsRunning((prev) => !prev);
    const nextState = !isRunning;

    // --- 🧩 Dummy speech activation for Safari ---
    if (!isRunning && currentCall === null && speechUtteranceRef.current) {
      const dummyUtterance = new SpeechSynthesisUtterance(" ");
      window.speechSynthesis.speak(dummyUtterance);
    }

    // --- 🗣️ Play message based on language ---
    if (language === "Arabic") {
      // Play pre-generated audio files
      const fileName = nextState ? "start_game.mp3" : "pause_game.mp3";
      const audio = new Audio(`/game/arabic/${fileName}`);
      audio.play().catch((err) => console.warn("Audio play blocked:", err));
    } else {
      // English: Use Web Speech API
      const message = nextState ? "The game has started" : "The game is paused";
      ...
      // Optional: pick matching English voice
      const voices = window.speechSynthesis.getVoices();
    ...
    <div className="w-screen h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 p-8 text-white font-sans overflow-hidden">
      ...
      <div className="flex space-x-8 h-[calc(100vh-160px)]">
        ...
        <div className="w-80 bg-white/5 backdrop-blur-md rounded-2xl p-6 flex flex-col justify-between shadow-xl border border-white/10">
          <div>
            <div className="text-lg mb-3 text-white/70 font-semibold">
          ...
          {/* Auto/Manual */}
          <div
            className="mt-6 mb-2 p-4 border border-white/20 rounded-md bg-white/5 max-w-md w-full"
            style={{ minWidth: 0 }}
          >
        ...
        <div className="flex-1 p-6 rounded-2xl bg-white/5 backdrop-blur-md shadow-xl border border-white/10 overflow-y-auto scrollbar-hide">
          <div className="grid grid-cols-16 gap-2 text-center font-bold text-white text-base">
          ...
          <div>
            <div className="bg-gradient-to-br from-purple-800 to-blue-800 text-yellow-300 rounded-xl p-6 text-center mt-10 text-9xl font-extrabold tracking-widest shadow-2xl animate-pulse-once border border-purple-700">
              {currentCall
